...
## 開発・デプロイ
開発・デプロイはDocker Composeを利用して行います。
事前にDockerおよびDocker Composeがインストールされていることを確認してください。
Composeでビルドを行う場合はbuildxプラグインが必要です。 未導入の場合は下記を参考にインストールし、必要に応じて`docker buildx create --use`でビルダーを作成してください。

### buildxの導入(主要OS)
- Windows: Docker Desktopに含まれる
- macOS: Docker Desktopに含まれる
- Ubuntu: Docker公式リポジトリでDockerを入れている場合は`sudo apt-get install docker-buildx-plugin`で導入
導入後は`docker buildx version`で確認し、必要に応じて`docker buildx create --use`を実行してください。
