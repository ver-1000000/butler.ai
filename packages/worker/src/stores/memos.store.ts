...
export class MemosStore {
  ...
  async data(): Promise<Omit<StoreResult<Record<string, string>>, 'key'>> {
    const rows   = this.db.prepare('SELECT key, value FROM memos ORDER BY key').all() as { key: string; value: string }[];
  ...
  async get(key: string): Promise<StoreResult<string | null>> {
    const row    = this.db.prepare('SELECT value FROM memos WHERE key = ?').get(key) as { value: string } | undefined;
  ...
  async set(key: string, value: string): Promise<StoreResult<string>> {
    this.db.prepare('INSERT INTO memos (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value').run(key, value);
  ...
  async del(key: string): Promise<StoreResult<string | null>> {
    ...
    if (value != null) { this.db.prepare('DELETE FROM memos WHERE key = ?').run(key); }
    return { pretty, key, value };
  }
}
